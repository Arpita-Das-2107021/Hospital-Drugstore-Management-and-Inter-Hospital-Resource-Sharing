...
Generates random pharmacy sales data for Bangladesh (Joypurhat district).
...
# ── 1. Pharmacies ──────────────────────────────────────────────────────────────
# 20 pharmacies spread across Joypurhat district, Bangladesh
PHARMACIES = [
...
# Dengue hits Joypurhat Sadar pharmacies in week 3
...
def get_multiplier(pharmacy, medicine_name, day):
    """Return sales multiplier if an outbreak affects this pharmacy/medicine."""
    for ob in OUTBREAKS:
        if (pharmacy["upazila"] in ob["upazilas"]
...
with open(f"{out}/pharmacies.csv", "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=["pharmacy_id","name","upazila","lat","lon"])
    w.writeheader(); w.writerows(PHARMACIES)
print(f"✓ pharmacies.csv  — {len(PHARMACIES)} pharmacies")

with open(f"{out}/sales.csv", "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=["date","pharmacy_id","medicine_name","quantity_sold","upazila"])
    w.writeheader(); w.writerows(sales_rows)
print(f"✓ sales.csv       — {len(sales_rows):,} records ({NUM_DAYS} days × {len(PHARMACIES)} pharmacies)")
