...
print(f"✓ pharmacies.csv        — {len(PHARMACIES)} pharmacies across 5 Khulna upazilas")

# sales.csv
with open(f"{OUT}/sales.csv", "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=["date","pharmacy_id","medicine_name",
                                       "quantity_sold","upazila"])
    w.writeheader()
    w.writerows(sales_rows)
print(f"✓ sales.csv             — {len(sales_rows):,} records "
      f"({NUM_DAYS} days × {len(PHARMACIES)} pharmacies × {len(MEDICINES)} medicines)")
