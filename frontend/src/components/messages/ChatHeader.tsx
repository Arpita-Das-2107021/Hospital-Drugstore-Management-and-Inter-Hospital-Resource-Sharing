...
import { Conversation, OnlineStatus, Employee } from '@/types/healthcare';
...
const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  if (!conversation) {
    return (
      <div className={cn("border-b p-4 h-16 flex items-center justify-center", className)}>
  ...
  const getConversationSubtitle = () => {
    ...
    if (conversation.type === 'case' && conversation.caseId) {
      const otherParticipant = conversation.participants.find(p => p.id !== '1');
      return `${conversation.caseId} • ${otherParticipant?.hospital}`;
    }
    
    const otherParticipant = conversation.participants.find(p => p.id !== '1');
    const status = onlineStatus?.[otherParticipant?.id || ''];
  ...
  const isOtherParticipantOnline = () => {
    const otherParticipant = conversation.participants.find(p => p.id !== '1');
    return onlineStatus?.[otherParticipant?.id || '']?.isOnline || false;
  };
  ...
  return (
    <TooltipProvider>
      <div className={cn("border-b p-4 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60", className)}>
            ...
            <div className="relative">
              <Avatar className="h-10 w-10">
                ...
                <AvatarFallback className={cn(
                  ...
                  {conversation.type === 'private' && getConversationAvatar()}
                </AvatarFallback>
              </Avatar>
