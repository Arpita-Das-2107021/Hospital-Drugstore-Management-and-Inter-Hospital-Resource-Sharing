...
const NewMessageModal: React.FC<NewMessageModalProps> = ({
  ...
  const filteredUsers = useMemo(() => {
    ...
    const currentHospital = (currentUser.hospital || '').toLowerCase().trim();
    let filtered = allUsers.filter((user) => {
      const sameId = !!currentUser.id && user.id === currentUser.id;
      const sameEmail =
        !!currentEmail &&
        !!user.email &&
        user.email.toLowerCase().trim() === currentEmail;
      const sameNameAndHospital =
        !!currentName &&
        !!user.name &&
        user.name.toLowerCase().trim() === currentName &&
        (!!currentHospital
          ? (user.hospital || '').toLowerCase().trim() === currentHospital
          : true);
      return !sameId && !sameEmail && !sameNameAndHospital;
    });
    ...
    return filtered;
  }, [allUsers, currentUser.id, currentUser.email, currentUser.name, currentUser.hospital, selectedHospital, selectedRole, searchQuery]);
    ...
    setSelectedParticipants((prev) =>
      prev.filter((user) => {
        ...
        const sameEmail =
          !!currentUser.email &&
          !!user.email &&
          user.email.toLowerCase().trim() === currentUser.email.toLowerCase().trim();
