// frontend/src/pages/system-admin/StaffInvitations.tsx
...
const StaffInvitations: React.FC = () => {
  const { user } = useAuth();
  const canManageInvitations = hasAnyPermission(user, ['platform:user.view', 'platform:user_role.assign', 'platform:role.assign', 'platform:role.manage']);
  const canManageAcrossHospitals = hasAnyPermission(user, ['platform:user.view', 'platform:user_role.assign', 'platform:hospital.view']);
  ...
  useEffect(() => {
    fetchData();
  }, [statusFilter, searchTerm, canManageAcrossHospitals]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (canManageAcrossHospitals) {
        ...
        setHospitals(hospitalsList.map((hospital) => ({ id: String(hospital.id), name: hospital.name || hospital.hospital_name || 'Healthcare' })));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (canManageAcrossHospitals && formData.hospital) (invitationData as unknown).hospital = formData.hospital;
  ...
  return (
    <AppLayout title="Staff Invitations"
      // subtitle="Invite and manage staff members across healthcare facilities"
    >
      <div className="max-w-7xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-white">Staff Invitations</h1>
        <p className="text-muted-foreground mt-1">Invite and manage staff members across the platform</p>
      </div>
        ...
        <div className="rounded-lg p-6 mb-6 border border-muted/40 bg-muted/10">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFormData((prev) => ({
                    ...
                    hospital: canManageAcrossHospitals ? (selected?.hospital || prev.hospital) : prev.hospital,
                  }));
                }}
            ...
            {canManageAcrossHospitals && (
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Healthcare Facility</label>
                <select
                  name="hospital"
