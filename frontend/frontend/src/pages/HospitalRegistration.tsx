...
function LocationPickerMarker({
  ...
  const position: [number, number] = Number.isFinite(lat) && Number.isFinite(lng) ? [lat, lng] : fallback;
...
const HospitalRegistration = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.api_base_url && !isValidHttpUrl(formData.api_base_url)) {
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">A platform administrator will review your hospital's registration details. This typically takes 1–2 business days.</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="font-medium flex items-center gap-1"><Mail className="h-4 w-4" /> Invitation Email Sent on Approval</p>
                        <p className="text-sm text-muted-foreground">Once approved, a <strong>password setup email</strong> will be sent to the <strong>hospital admin email</strong> <strong>{registration.admin_email || formData.admin_email}</strong>. This email contains a secure link to set up your administrator account and password.</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Click the invitation link in the email to create your secure password and activate your hospital admin account.</p>
                    </div>
                  </div>
              ...
              <div className="flex gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-800 dark:text-blue-200 mt-1">No account has been created yet. Credentials are created only after approval, and the setup link is sent to the hospital admin email <strong>{registration.admin_email || formData.admin_email}</strong>.</p>
                </div>
              </div>

              <div className="flex gap-3 pt-4">
                <Button className="flex-1" onClick={() => window.location.assign('/login')}>Go to Login</Button>
                <Button variant="outline" className="flex-1" onClick={() => window.location.assign('/register')}>Register Another Hospital</Button>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <div className="w-full">
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Building2 className="h-5 w-5" /> Hospital Registration
              </CardTitle>
              <CardDescription>
                Complete the hospital information and API configuration below.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit}>
                ...
                {/* Hospital Info Section */}
                  <div className="space-y-4">
                    <p className="text-sm font-medium">Hospital Information</p>
                    ...
                    <div className="space-y-3 rounded-lg border p-4 bg-muted/20">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={() => setShowMapPicker((prev) => !prev)}>
                      ...
                      {showMapPicker && (
                        <div className="overflow-hidden rounded-md border">
                          <MapContainer
                            center={[
                              Number(formData.latitude) || 23.8103,
                              Number(formData.longitude) || 90.4125,
                            ]}
                            zoom={6}
                      ...
                      <p className="text-xs text-muted-foreground">
                        You can either enter coordinates manually or click on the map to set them.
                      </p>
                    </div>

                    <div className="border rounded-lg p-4 bg-muted/30 space-y-4">
                      <p className="text-sm font-medium">Hospital Admin Information</p>
                ...
                {/* API Configuration Section */}
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Base API URL is optional. Provide it if your system supports API sync.
                    </p>
                    ...
                    {formData.api_base_url && (
                      <>
                        ...
                        {(formData.api_auth_type === 'basic' || formData.api_auth_type === 'bearer') && (
                          <div className="grid grid-cols-2 gap-4">
                            <div>
