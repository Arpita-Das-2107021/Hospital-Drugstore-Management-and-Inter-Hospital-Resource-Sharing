...
Builds a healthcare network graph where:
  - Each healthcare = node
  - Edge between two healthcares if distance < EDGE_RADIUS_KM
...
EDGE_RADIUS_KM = 10.0   # connect healthcares within 10 km
...
# ── Load data ─────────────────────────────────────────────────────────────────
def load_healthcares():
    healthcares = {}
    with open(f"{DATA_DIR}/healthcares.csv") as f:
        for row in csv.DictReader(f):
            healthcares[row["healthcare_id"]] = {
                "name":    row["name"],
                "upazila": row["upazila"],
                "lat":     float(row["lat"]),
                "lon":     float(row["lon"]),
...
def load_sales():
    """Returns {date: {healthcare_id: {feature_idx: total_qty}}}"""
    ...
    with open(f"{DATA_DIR}/sales.csv") as f:
        for row in csv.DictReader(f):
            ...
            sales[row["date"]][row["healthcare_id"]][feat_idx] += float(row["quantity_sold"])
    return sales

# ── Build static graph edges ──────────────────────────────────────────────────
def build_edges(healthcares):
    """
    Returns list of (i, j, distance_km) for all healthcare pairs within radius.
    Nodes are indexed by sorted healthcare_id order.
    """
    ph_ids = sorted(healthcares.keys())
    edges  = []
    for i, id_a in enumerate(ph_ids):
        for j, id_b in enumerate(ph_ids):
            if j <= i:
                continue
            dist = haversine_km(
                healthcares[id_a]["lat"], healthcares[id_a]["lon"],
                healthcares[id_b]["lat"], healthcares[id_b]["lon"]
            )
...
def build_graph_sequence(num_days=60):
    ...
    Each snapshot = {
        "day":        int,
        "date":       str,
        "node_ids":   [healthcare_id, ...],          # fixed order
        "node_feats": [[f0..f4], ...],              # one row per healthcare
    ...
    """
    healthcares = load_healthcares()
    sales      = load_sales()
    ph_ids, edges = build_edges(healthcares)
...
def attach_labels(snapshots, healthcares):
    """
    Reads outbreaks_ground_truth.csv and marks each snapshot+healthcare
    ...
    for snap in snapshots:
        ...
        for ph_id in snap["node_ids"]:
            upazila = healthcares[ph_id]["upazila"]
            active  = any(
...
def print_summary(snapshots, ph_ids, edges):
    ...
    print("═"*55)
    print(f"  Nodes (healthcares) : {len(ph_ids)}")
...
if __name__ == "__main__":
    print("Building healthcare graph network...")
    healthcares        = load_healthcares()
    ph_ids, edges     = build_edges(healthcares)
    snapshots         = build_graph_sequence(num_days=60)
    snapshots         = attach_labels(snapshots, healthcares)
