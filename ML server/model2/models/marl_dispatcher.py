...
When the ST-GNN flags an outbreak, agents automatically decide:
  - Which healthcare to send stock to
  - How much to send
...
class HealthcareNeed:
    """A healthcare flagged as needing restock due to predicted outbreak."""
    def __init__(self, healthcare_id, upazila, lat, lon,
...
class MARLDispatcher:
    ...
    def policy(self, depot, needs):
        ...
        return {
            "destination": best_need.healthcare_id,
    ...
    def run_episode(self, needs):
        ...
        needs_before = [HealthcareNeed(
            n.healthcare_id, n.upazila, n.lat, n.lon,
        ...
        for round_num in range(3):
            ...
            for depot in self.depots:
                ...
                dest_need = next((n for n in needs
                                   if n.healthcare_id == action["destination"]
...
def run_demo():
    ...
    # Healthcare needs flagged by the ST-GNN (simulated output)
    needs = [
        HealthcareNeed("PH001", "Joypurhat Sadar", 24.898, 89.017,
                     "Paracetamol 500mg",  current_stock=20,  predicted_demand=180),
        HealthcareNeed("PH002", "Joypurhat Sadar", 24.902, 89.021,
                     "Cetirizine 10mg",    current_stock=10,  predicted_demand=120),
        HealthcareNeed("PH008", "Kalai",           25.017, 89.063,
                     "ORS Sachet",         current_stock=15,  predicted_demand=200),
        HealthcareNeed("PH009", "Kalai",           25.022, 89.058,
                     "Metronidazole 400mg",current_stock=5,   predicted_demand=90),
        HealthcareNeed("PH011", "Khetlal",         24.988, 89.115,
                     "ORS Sachet",         current_stock=8,   predicted_demand=160),
        HealthcareNeed("PH014", "Panchbibi",       25.050, 89.150,   # underserved
    ...
    # ── Print dispatch plan ───────────────────────────────────────────────────
    print(f"  {'From Depot':<22} {'→ To Healthcare':<10} {'Medicine':<22} {'Qty':>5}  {'km':>6}  Round")
    print("  " + "─"*78)
    for d in dispatches:
    ...
    for need in needs:
        ...
        print(f"    {need.healthcare_id} {need.upazila:<20} [{bar}] {pct:5.1f}%{flag}")
